...
public class SeleniumUtil {
	...
	public void click(WebElement element) {
		...
		try {
			...
			logger.error("The element you clicked:[" + element.toString()
					+ "] is no longer exist!");
			Assert.fail("The element you clicked:[" + element.toString()
					+ "] is no longer exist!");
		} catch (Exception e) {
		...
		logger.info("点击元素 [" + element.toString() + "] " + element.getText());
	}
	...
	public String getTitle() {
		logger.info("当前页面标题为[" + driver.getTitle().trim() +"]");
	...
	public void FindElementUtil3TimesTry(int timeOut, final By By) {
		...
		while (flag) {
			if (notFindTimes == 3) {
				logger.error("尝试了3次查找都未查找到元素：" + By
						+ "请检查是不是网络或者网站性能问题（响应速度不够）");
			...
			try {
				...
				logger.warn("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "],这是第"
						+ (notFindTimes + 1) + "次查找！");
				notFindTimes++;
				if (notFindTimes < 3) {
					refresh();
	 ...
	 * 判断编辑框是不是可编辑
	 * */
	public boolean isInputEdit(WebElement element) {
		boolean isEnabled = false;
		if (element.isEnabled()) {
			logger.info("The element: [" + getLocatorByElement(element, ">")
					+ "] is enabled");
			isEnabled = true;
		} else if (element.isEnabled() == false) {
			logger.warn("The element: [" + getLocatorByElement(element, ">")
					+ "] is not enabled");
	 ...
	 * 切换frame - 根据页面元素定位
	 * */
	public void switchFrameToFrame(By byElement1,By byElement2,String content) {
		try {
			logger.info("Start switching to frame [" + byElement1 + " To " + byElement2 + "]");
			WebElement contentElement = driver.switchTo()
					.frame(findElementBy(byElement1)).findElement(byElement2);
			logger.info("Switch to frame [" + byElement1 + " To " +byElement2 + "] successed");
			contentElement.sendKeys(content);
		} catch (Exception e) {
			logger.info("Switch to frame [" + byElement1 + " To " +byElement2 + "] failed");
			Assert.fail("Switch to frame [" + byElement1 + " To " +byElement2 + "] failed");
		}
		logger.info("输入：[" + content + "] 到TextBox中");
	}
	...
	public boolean doesCheckboxSelected(By elementLocator) {
		if (findElementBy(elementLocator).isSelected() == true) {
			logger.info("CheckBox: "
					+ getLocatorByElement(findElementBy(elementLocator), ">")
					+ " 被勾选");
			return true;
		} else
			logger.info("CheckBox: "
	 ...
	 *              seleniumm不能直接操作，需要借助http://modifyusername
	 *              :modifypassword@yoururl 这种方法
	 * 
	 * */
	 ...
	 * @param password 密码
	 */
	public void login163Mail(String username, String password) {
		FindElementUtil3TimesTry(30, By.xpath("//*[@id='loginDiv']/iframe"));
		switchFrame(By.xpath("//*[@id='loginDiv']/iframe"));
		FindElementUtil3TimesTry(30, By.name("email"));
		FindElementUtil3TimesTry(30, By.name("password"));
		FindElementUtil3TimesTry(30, By.id("dologin"));
		clear(By.name("email"));
	 ...
	 * 			所要查找的文本内容关键字是否存在
	 */
	public void isContentDisable(String content,int timeOut) {
		
		logger.info("等待文本关键字[ " + content + " ]是否出现");
		WebElement contentDisable = null;
		boolean result;
		try {
			contentDisable = (new WebDriverWait(driver,timeOut)).
					until(ExpectedConditions.presenceOfElementLocated
							(By.xpath("//*[contains(text(),'" + content + "')]")));
			logger.info("当前文本信息为[ " + contentDisable.getText() +" ]");
			//boolean result =contentDisable.getText().contains(content); // contentDisable.isDisplayed();
			result = contentDisable.isDisplayed();
			Assert.assertTrue(result);
			logger.info("发送邮件成功");
		} 
		catch (TimeoutException e) 
		{			
			logger.warn("找不到该元素[ " + contentDisable + " ]或该元素不存在");
		} 
		catch (Exception e)
		{
			result = false;
	...
	/**
	 * @description 
	 * @param findElementsBy
	 * @param index
	 */
	public void clickCheckBoxs(List<WebElement> findElementsBy,int index) {
		//若index的值小于复选框个数，则指定勾选某个复选框；若大于，则勾选全部
		if (findElementsBy.size() > index){
			findElementsBy.get(index).click();
		}else{
			for (WebElement element : findElementsBy){
				element.click();
			}
		}
	}
